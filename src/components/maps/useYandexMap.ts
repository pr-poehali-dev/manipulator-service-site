  ...
  useEffect(() => {
    const loadYandexMapScript = () => {
      ...
      try {
        ...
        script.onerror = (e) => setError(new Error('Не удалось загрузить Яндекс.Карты'));
        
        document.body.appendChild(script);
      } catch (err) {
        setError(err instanceof Error ? err : new Error('Неизвестная ошибка при загрузке карт'));
      }
    };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          console.log("Геолокация недоступна, будет использовано местоположение по умолчанию");
        }
      );
    }
  ...
  useEffect(() => {
    ...
    window.ymaps.ready(() => {
      try {
        ...
        setError(err instanceof Error ? err : new Error('Ошибка инициализации карты'));
      }
    });
  }, [isLoaded, latitude, longitude, city, address, additionalInfo, balloonTitle, mapOptions]);
